...
def pagina_gerador(request):
    ...
    if request.method == 'POST' and formset.is_valid():
        for form in formset:
            ...
            caminho_narrador_input, timepoints, caminho_legenda_ass, caminho_imagem_texto, caminho_tela_final_img = None, None, None, None, None
            ...
            if usar_narrador and data.get('narrador_texto'):
                ...
                caminho_narrador_input, timepoints = gerar_audio_e_tempos(
                    data['narrador_texto'], data['narrador_voz'], data['narrador_velocidade'], data['narrador_tom'], obter_tempos=obter_tempos
                )
            ...
            if texto_tela_final:
                ...
                caminho_tela_final_img = create_text_image(texto_tela_final, '#FFFFFF', opcoes_tela_final, 'centro')
            ...
            try:
                ...
                cmd_etapa1 = ['ffmpeg', '-y']
                if data.get('loop_video', True): cmd_etapa1.extend(['-stream_loop', '-1', '-i', caminho_video_input])
                else: cmd_etapa1.extend(['-i', caminho_video_input])
                
                inputs_adicionais = []
                if caminho_imagem_texto: inputs_adicionais.append(caminho_imagem_texto)
                inputs_adicionais.append(caminho_musica_input)
                if caminho_narrador_input: inputs_adicionais.append(caminho_narrador_input)
                
                for f in inputs_adicionais: cmd_etapa1.extend(['-i', f])
                ...
                if not usar_narrador: cmd_etapa1.extend(['-t', str(data.get('duracao_segundos', 30))])
                
                cmd_etapa1.extend(['-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-shortest'])
                ...
                for path in [caminho_narrador_input, caminho_legenda_ass, caminho_imagem_texto, caminho_tela_final_img, caminho_video_temp, caminho_tela_final_video, lista_concat_path]:
